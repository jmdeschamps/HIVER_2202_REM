...
class Vue():
    def __init__(self, controleur, urlserveur, monnom, testdispo):
        ...
        for caseX in range(0, len(self.modele.cartecase), 1):
            for caseY in range(0, len(self.modele.cartecase), 1):
                self.canevas.create_rectangle(caseX * 72, caseY * 72, caseX * 72 + 72, caseY * 72 + 72,
                                              fill=couleur_cases[self.modele.cartecase[caseY][caseX].montype])
    ...
    def afficher_depart(self):
        ...
        for j in self.modele.joueurs.keys():
            for i in self.modele.joueurs[j].batiments["maison"].keys():
                village_initiale = self.modele.joueurs[j].batiments["maison"][i]
                ...
                for zdc in village_initiale.zone_controle:
                    if zdc.x == case.x and zdc.y == case.y:
                        ...
                        self.canevas.create_rectangle(zdc.x * 72, zdc.y * 72, zdc.x * 72 + 72, zdc.y * 72 + 72,
                                                      fill="orange red", stipple="gray12")
    ...
    def afficher_batiment(self, joueur, batiment):
        ...
        # Affichage de la zone de controle
        #        for i in self.modele.joueurs[joueur].batiments:
        #            for j in self.modele.joueurs[joueur].batiments[i]:
        #                if self.modele.joueurs[joueur].batiments[i][j] == batiment:
        #                    pass
        #                else:
        #                    for zdc in batiment.zone_controle:
        #                        if (zdc.x == batiment.case.x and zdc.y == batiment.case.y) or \
        #                                (zdc.x == self.modele.joueurs[joueur].batiments[i][j].case.x and zdc.y == self.modele.joueurs[joueur].batiments[i][j].case.y):
        #                            pass
        #                        else:
        #                            self.canevas.create_rectangle(zdc.x * 72, zdc.y * 72, zdc.x * 72 + 72, zdc.y * 72 + 72,
        #                                                          fill="orange red", stipple="gray12")
    ...
    def afficher_jeu(self):
        ...
        for j in self.modele.joueurs.keys():
            ...
            if j == self.controleur.monnom:
                ...
                self.infohud["Or"][0].set(self.modele.joueurs[j].Or)
    ...
    def batir_artefact(self, evt):
        ...
        vals = self.controleur.trouver_valeurs()

        or_du_joueur = self.modele.joueurs[self.monnom].Or
        cout_du_batiment = vals[nomsorte]["Or"]

        if or_du_joueur < cout_du_batiment:
            self.action.btnactif.config(bg="SystemButtonFace")
    ...
    def construire_batiment(self, evt):
        mestags = self.canevas.gettags(CURRENT)
        # not mestags
        if self.action.persochoisi and self.action.prochaineaction:
