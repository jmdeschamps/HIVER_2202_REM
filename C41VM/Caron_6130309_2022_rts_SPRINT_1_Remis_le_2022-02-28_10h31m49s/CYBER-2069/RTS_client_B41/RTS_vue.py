...
##### FONCTION DU LOBBY #############
    ...
    def initialiser_avec_modele(self):
        ...
        #######################################################################################
        self.creer_cadre_marchand(coul[0]+"_",["maison","caserne","abri","usineballiste"])
        self.creer_chatter()
        #self.creer_quetes()
...
######################################################################################
    def creer_cadre_marchand(self,coul,artefacts): # CHANGEMENT ANTO
        self.cadremarchand=Frame(self.canevasaction)
        for i in artefacts:
            btn=Button(self.cadremarchand,text=i,image=self.images[coul+i])
            btn.bind("<Button>",self.batir_artefact)
            btn.pack()
        print("Je suis le marchand")
###########################################################################
