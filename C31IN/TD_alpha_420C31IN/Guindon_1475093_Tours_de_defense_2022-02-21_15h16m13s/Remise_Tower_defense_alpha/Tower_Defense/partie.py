...
class Partie:
    ...
    def attaquer_forteresse(self, creep):
        ...
        if self.vie > 0:
            self.message = "Vie : " + str(self.vie)
        else:
            self.message = "Vie : 0"
        self.parent.update_commencer(self.message)
        ...
        if self.vie <= 0:
            self.game_over()
            self.parent.parent.vue.creer_page_fin_partie()
    ...
    def fin_vague(self):
        ...
        self.argent += bonus *5
        self.parent.parent.vue.update_argent()
        ...
        if self.ennemi_vague[self.vague] < 10:
            self.nbr_creeps_un = math.floor(self.nbr_creeps_vague/4)
            self.nbr_creeps_deux = 0
            self.nbr_creeps_trois = 0
            self.nbr_creeps_quatre = 0
    ...
    def retirer_argent(self, type):
        ...
        if type == "FEU":
            ...
            if self.argent > 220:
                self.argent -= 220  # TourGlace.prix
                self.parent.update_argent()
                return True
            ...
            if self.argent > 275:
                self.argent -= 275  # TourGlace.prix
                self.parent.update_argent()
                return True
            ...
            if self.argent > 335:
                self.argent -= 335  # TourGlace.prix
                self.parent.update_argent()
                return True
            else :
            ...
            if type_tour == "FEU":
                ...
                for tour in self.tours_poison:
                    ...
                    if ((creep.pos_x - tour.x) ** 2) + ((creep.pos_y - tour.y) ** 2) < (tour.portee_attaque ** 2):
                        ...
                        #     tour.creeps_attaquer.append(creep)
                        #     creep.est_attaquer = True
                        # if tour.nbr_attaques_en_cours < 5:
            ...
            if tour.attaque_en_cours:
                for projectile in tour.projectiles:
                    ...
                    if cible_a_retirer is not None:
                        ...
                        self.retirer_projectile(tour, projectile)
                    elif not projectile.cible.est_mobile:
                        self.retirer_projectile(tour, projectile)
        for tour in self.tours_mitraille:
            for projectile in tour.projectiles:
                cible_a_retirer = projectile.parcours_la_zone_de_jeu()
